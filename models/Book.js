...
class Book {
    //file直接从电子书文件中解析出 Book 对象，第二种是从 data 对象中生成 Book 对象
    ...
    createBookFromFile(file) {
        ...
        const { destination, filename, mimetype = MINE_TYPE_EPUB, originalname } = file
        //电子书的文件后缀名
        ...
        this.originalName = originalname //电子书原件的原名
    }
    ...
    parseContents(epub) {
        function getNcxFilePath() {
            ...
            // console.log('ncx', spine.toc, ncx, id, manifest[id].href)
    ...
    reset() {
        ...
        if (Book.pathExists(this.unzipPath)) {
            // recursive表示迭代删除,低版本node不支持该属性
            fs.rmdirSync(Book.genPath(this.unzipPath), { recursive: true })
        }
    }

    // genPath 是 Book 的一个属性方法，使用 es6 的 static 属性来实现
    // 绝对路径
    static genPath(path) {
        if (!path.startsWith('/')) {
            path = `/${path}`
    ...
    // edit里的fileName,封装getContentsTree
